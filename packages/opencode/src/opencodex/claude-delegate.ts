...
export type Swarm = {
  ...
  run: (input: { role: string; prompt: string }) => Effect.Effect<Result, unknown>
}
...
export function capability(bridge: EffectBridge.Shape, delegate: Swarm) {
  return {
    ...
    run: (input: { role: string; prompt: string; signal?: AbortSignal }) => {
      if (input.signal?.aborted) return Promise.resolve(failure("cancelled"))
      return bridge
        .promiseExit(
          delegate.run({ role: input.role, prompt: input.prompt }),
