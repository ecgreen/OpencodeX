...
export interface Interface {
  ...
  readonly fromDirectory: (directory: string) => Effect.Effect<{ project: Info; sandbox: string; opencodexFolders: string[] }>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const fromDirectory = Effect.fn("Project.fromDirectory")(function* (directory: string) {
      ...
      const matched = yield* OpencodeXProjectFolder.findFolder(db, directory)

      // Phase 2: upsert
      const projectID = matched?.project_id ?? ProjectV2.ID.make(data.id)
      if (!matched) yield* migrateProjectId(data.previous ? ProjectV2.ID.make(data.previous) : undefined, projectID)
      ...
      const result: Info = {
        ...
        vcs: matched ? existing.vcs : (data.vcs?.type ?? fakeVcs),
      ...
      if (projectID !== ProjectV2.ID.global && !matched) {
        yield* db
          .update(SessionTable)
      ...
      if (projectID !== ProjectV2.ID.global && !matched && data.vcs?.type === "git") {
        yield* projectV2.commit({ store: data.vcs.store, id: data.id })
      }
      return {
        project: result,
        sandbox: matched ? matched.path : data.vcs ? data.directory : worktree,
        opencodexFolders: (
          yield* (matched
            ? OpencodeXProjectFolder.listFolders(db, matched.opencodex_project_id)
            : OpencodeXProjectFolder.listFoldersForOpencodeProjects(db, [projectID]))
        ).map((item) => item.path),
      }
    })
