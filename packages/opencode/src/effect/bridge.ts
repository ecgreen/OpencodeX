...
export interface Shape {
  readonly promise: <A, E, R>(effect: Effect.Effect<A, E, R>, signal?: AbortSignal) => Promise<A>
  ...
  return Effect.gen(function* () {
    ...
    return {
      promise: <A, E, R>(effect: Effect.Effect<A, E, R>, signal?: AbortSignal) =>
        restoreWorkspace(workspace, () => Effect.runPromise(wrap(effect), signal ? { signal } : undefined)),
