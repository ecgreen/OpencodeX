...
export async function resolvePathPluginTarget(spec: string, directory = process.cwd()) {
  const raw = spec.startsWith("file://") ? fileURLToPath(spec) : spec
  const file = path.isAbsolute(raw) || /^[A-Za-z]:[\\/]/.test(raw) ? raw : path.resolve(directory, raw)
  const stat = await Filesystem.statAsync(file)
  if (!stat?.isDirectory()) {
...
export async function resolvePluginTarget(spec: string, directory = process.cwd()) {
  if (isPathPluginSpec(spec)) return resolvePathPluginTarget(spec, directory)
  const hit = parse(spec)
