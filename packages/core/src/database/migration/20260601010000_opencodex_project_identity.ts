...
export default {
  id: "20260601010000_opencodex_project_identity",
  up(tx) {
    return Effect.gen(function* () {
      ...
      const columns = new Set(
        (yield* tx.all<{ name: string }>(sql`PRAGMA table_info(${sql.identifier("opencodex_project_folder")})`)).map(
          (column) => column.name,
        ),
      )
      if (!columns.has("opencodex_project_id")) {
      ...
      const pathColumn = (
        yield* tx.all<{ name: string; pk: number }>(sql`PRAGMA table_info(${sql.identifier("opencodex_project_folder")})`)
      ).find((column) => column.name === "path")
      if (pathColumn?.pk === 1) {
        yield* tx.run(`
          CREATE TABLE \`opencodex_project_folder_next\` (
            \`path\` text NOT NULL,
            ...
            CONSTRAINT \`fk_opencodex_project_folder_next_opencodex_project_id_opencodex_project_id_fk\` FOREIGN KEY (\`opencodex_project_id\`) REFERENCES \`opencodex_project\`(\`id\`) ON DELETE CASCADE,
            CONSTRAINT \`fk_opencodex_project_folder_next_project_id_project_id_fk\` FOREIGN KEY (\`project_id\`) REFERENCES \`project\`(\`id\`) ON DELETE CASCADE
          );
        `)
        yield* tx.run(`
          INSERT OR IGNORE INTO \`opencodex_project_folder_next\`
        ...
        yield* tx.run(`ALTER TABLE \`opencodex_project_folder_next\` RENAME TO \`opencodex_project_folder\`;`)
        yield* tx.run(
      ...
      yield* tx.run(`
        CREATE TABLE IF NOT EXISTS \`opencodex_project_session\` (
      ...
      `)
      yield* tx.run(
        `CREATE INDEX IF NOT EXISTS \`opencodex_project_session_project_idx\` ON \`opencodex_project_session\` (\`opencodex_project_id\`);`,
      )
    })
  },
