...
export default {
  id: "20260731034851_lying_enchantress",
  up(tx) {
    return Effect.gen(function* () {
      ...
      `)
      yield* tx.run(`CREATE INDEX \`opencodex_goal_edge_goal_idx\` ON \`opencodex_goal_edge\` (\`goal_id\`);`)
      yield* tx.run(`CREATE INDEX \`opencodex_goal_edge_to_idx\` ON \`opencodex_goal_edge\` (\`to_node_id\`);`)
      yield* tx.run(`CREATE INDEX \`opencodex_goal_node_goal_idx\` ON \`opencodex_goal_node\` (\`goal_id\`);`)
      yield* tx.run(`CREATE INDEX \`opencodex_goal_node_parent_idx\` ON \`opencodex_goal_node\` (\`parent_node_id\`);`)
      yield* tx.run(`CREATE INDEX \`opencodex_goal_node_job_idx\` ON \`opencodex_goal_node\` (\`job_id\`);`)
      yield* tx.run(`CREATE INDEX \`opencodex_goal_node_session_idx\` ON \`opencodex_goal_node\` (\`session_id\`);`)
      yield* tx.run(`CREATE INDEX \`opencodex_goal_node_status_idx\` ON \`opencodex_goal_node\` (\`status\`);`)
      yield* tx.run(`CREATE INDEX \`opencodex_goal_project_idx\` ON \`opencodex_goal\` (\`opencodex_project_id\`);`)
      yield* tx.run(`CREATE INDEX \`opencodex_goal_session_idx\` ON \`opencodex_goal\` (\`owner_session_id\`);`)
      yield* tx.run(`CREATE INDEX \`opencodex_goal_swarm_idx\` ON \`opencodex_goal\` (\`swarm_id\`);`)
      yield* tx.run(`CREATE INDEX \`opencodex_goal_status_idx\` ON \`opencodex_goal\` (\`status\`);`)
      yield* tx.run(`CREATE INDEX \`opencodex_goal_updated_idx\` ON \`opencodex_goal\` (\`time_updated\`);`)
    })
  },
