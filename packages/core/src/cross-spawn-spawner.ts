  ...
  const spawn = (command: ChildProcess.StandardCommand, opts: NodeChildProcess.SpawnOptions) =>
    Effect.callback<readonly [NodeChildProcess.ChildProcess, ExitSignal, ExitSignal], PlatformError.PlatformError>(
      (resume) => {
        ...
        // 'close' reports the stdio streams closing, which is not the same
        // question as whether the process is gone: a detached grandchild
        // holding those pipes, or a reader torn down mid-stream, keeps it from
        // ever firing on a child that has already exited. 'exit' is the event
        // that means dead, so everything that only needs the process gone -
        // every kill path, and isRunning - waits on this deferred instead.
        // Waiting on 'close' there left a session cancel hanging for good on a
        // child it had itself SIGKILLed.
